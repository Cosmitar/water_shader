...
void main() {
  ...
  maskText.rgb = vec3(depth);
  // maskText.rgb = worldPosition.y - waterY < -0.5 ? vec3(1.0) : vec3(0.0);
  maskText.a = maskText.r;
  // maskText.a = remap(cameraNear, cameraFar, 0.0, 1.0, depth);

  gl_FragColor = maskText;
}
